...
- Los mockups de prenda (`svg_mock`/`svg_mock_back`) deben tener el cuerpo en BLANCO (fills cercanos a blanco) y las costuras/detalles en NEGRO/GRIS. El tintado lo hace `lib/recolorMock.ts` (helper `recolorMockSvg`) con las reglas del cliente: 1) elemento blanco (luma > 220) → color elegido; 2) elemento negro/gris oscuro (luma ≤ 60 y grisáceo: costuras, rayas, sombras) → se conserva; 3) si el color elegido es NEGRO → los negros se repintan a `#272727` para contraste. Aplica a TODOS los SVGs, incluye `rgb()`/`rgba()` con % y `style="fill:..."`, y excluye los `<defs>` (las líneas finas del jogger son negras en gradientes dentro de `<defs>` y NO se tinen). NO usar solo `currentColor` en los mockups (los templates de vecteezy no lo usan y quedaban en gris/blanco).
...
### Sesión 13 — 2026-08-12 (Reglas de tinte blancos/negros + `#272727` en todos los SVGs)

- Reporte del cliente: (1) en el jogger los "gradientes" NO son sombras grises sino líneas negras delgadas (no se deben teñir); (2) en la polo hay elementos que no colorean.
- Causa de los no-teñidos en la polo: el SVG actual en Supabase (g5) usa `fill="rgb(94.9%, 94.9%, 94.9%)"` (casi blanco) + `style="fill:#ffffff"` + grises medios (`rgb(70.1%)`, `rgb(50.2%)`, `#666666`, `#878483`) y azul oscuro (`rgb(15.7%, 18%, 28.6%)`, `#282e49`). El helper de la sesión 12 solo miraba `fill="#hex6"` → los `rgb(%)` y `style=` no se teñían.
- Rewrite de `lib/recolorMock.ts`: `parseColor()` acepta `#rgb`/`#rrggbb`/`rgb()`/`rgba()` con % o números y `white`/`black`. `buildReplacements()` escanea `fill|stroke|stop-color|color` en atributos Y en declaraciones `style`; aplica las reglas whiteshop (blanco → color; oscuro se conserva; si el color elegido es negro → negros a `#272727`). Los bloques `<defs>` se extraen a placeholders `@@recolorMockDefsN@@` y se reinsertan iguales (las líneas del jogger no se tocan). Sin caracteres de control (evita warning `no-control-regex`).
...
### Sesión 14b — 2026-08-13 (Fix: breadcrumb/header duplicados + recomendaciones eliminadas + layout móvil alineado con el CTA)

- Reporte del cliente (vista móvil de personalización): (1) se repetía el header — arriba "Inicio › nombre de la prenda" (breadcrumb) y más abajo el AppHeader con botón de retroceso + nombre; (2) aparecía la sección "QUIZÁS TAMBIÉN TE GUSTE" con 3 tarjetas; (3) la imagen y los controles (color/talla/elegir diseño) quedaban desalineados respecto al botón "Agregar al carrito" (que se veía más ancho) y, con el layout más alto, el botón "Elegir diseño" podía quedar tapado por el footer fijo.
- Fix 1 (`ProductPage.tsx`): eliminado el `<div className="breadcrumb">`. Queda solo `AppHeader showBack title` (icono de retroceso + nombre) como cabecera.
- Fix 2 (`ProductPage.tsx` + `App.css`): eliminada toda la sección `.recommendations` (estado, useEffect de ranking por tags, JSX y CSS) y el import de `recolorMockSvg`/`GarmentRow` que solo se usaba ahí. Ojo: `recolorMockSvg` sigue usándose en `HomePage`, `AdminGarmentForm`, `GarmentMock`.
